...
module DynamicReviewMapping   
 ...
 #--------------METHODS BELOW FOR STAGGERED DEADLINE ASSIGNMENT / Assignment with signup sheet-----------------
  ...
  def initialize_counts(people, review_count)
    people_review_count = Hash.new
    people.each {|person|
      people_review_count[person] = review_count
    }
    return people_review_count
  end

  def get_topic_count(topic_team_id)
  ...
  def check_mapping_team_and_user(mappings, users, team_reviewers_count, user_review_count, individual)
      ...
      for i in 0..mapping[1].size-1
        if mapping[1][i].nil?
          ...
          for j in (1..users.size*2)
            ...
            if mapping[1].index(users[random_index]).nil?
              if (team.nil? && !individual) || (mapping[0].to_i != users[random_index].to_i && indivdual)
    ...
    return mappings, team_reviewers_count, user_review_count
  end


  def check_user_topic(assignment, user, number_of_reviews, topic, topic_team_id, team_reviewers_count, mappings, user_review_count)
    ...
    return teams_to_assigned, user_review_count, team_reviewers_count, mappings
  end

  def get_updated_message_from_user(assignment, message, show_message, user_review_count)
  ...
  def meta_reviewers_check_mapping_team_and_user(assignment, mappings, users, team_reviewers_count, user_review_count)
    mappings.each {|mapping|
      #mapping[0]=team_id and mapping[1]=users(array) assigned for reviewing this team
      for i in 0..mapping[1].size-1
  ...
  def assign_reviewers_team(num_reviews)
    ...
    user_review_count = initialize_counts(users, number_of_reviews)

    #-------------initialize team_reviewers_count-------------
    team_reviewers_count = initialize_counts(contributors, reviews_per_team)
    ...
    for i in 1..users.size
      ...
      #Get topic count.
      topic_count = get_topic_count(topic_team_id)
      ...
      while user_review_count[user] != 0
        ...
        teams_to_assigned, user_review_count, team_reviewers_count, mappings = check_user_topic(@assignment, user, number_of_reviews, topic, topic_team_id, team_reviewers_count, mappings, user_review_count)
    ...
    individual = false;
    mappings, team_reviewers_count, user_review_count = check_mapping_team_and_user(mappings, users, team_reviewers_count, user_review_count, individual)
    ...
    message, @show_message = get_updated_message_from_user(@assignment, message, @show_message, user_review_count)
  ...
  def assign_metareviewers(num_review_of_reviews, assignment)
    ...
    #-------------initialize team_reviewers_count-------------
    team_reviewers_count = initialize_counts(contributors, reviews_per_user)
    ...
    for i in 1..users.size
      ...
      #Get topic count.
      topic_count = get_topic_count(topic_user_id)
    ...
    mappings, team_reviewers_count, user_review_count = meta_reviewers_check_mapping_team_and_user(@assignment, mappings, users, team_reviewers_count, user_review_count)
